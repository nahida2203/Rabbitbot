...
Bot.adapter.push(new class OneBotv11Adapter {
  id = "QQ"
  name = "OneBotv11"
  path = (process.env.ONEBOTV11_PATH ? String(process.env.ONEBOTV11_PATH).replace(/^\//, "") : this.name)
  echo = new Map
  timeout = 60000
  ...
  async makeMsg(msg) {
    // 统一成数组并扁平化，避免插件或上层包装产生的嵌套数组导致段类型缺失
    ...
    for (let i of flat) {
      ...
      if (typeof i !== "object") {
        ...
        // 兼容旧式段对象（如 {type:"at", qq:123}）为标准结构
        i = { type: i.type, data: { ...i, type: undefined } }
      }

      // 若依然缺少类型，兜底为文本，防止发送失败
      if (!i.type) {
        const text = typeof i?.data?.text === "string" ? i.data.text : Bot.String(i?.data ?? i)
      ...
      switch (i.type) {
        ...
        case "node":
          ...
          continue
        case "raw":
          // 透传原始数据，交由 OneBot 端自行判断；此处保持与现有行为一致
          i = i.data
          break
      }
