...
const pastelBlue = chalk.hex('#a5d8ff')
    const pastelPink = chalk.hex('#ffc0dd')
    const defid = pastelBlue(`[${this.globalConfig.align}]`)

    for (const method in chalk.logger) {
      if (typeof chalk.logger[method] === 'function') {
        if (method === 'warn' || method === 'error' || method === 'fatal') {
          chalk[method] = (...args) => chalk.logger[method](pastelPink(`[${this.globalConfig.align}]`), ...args)
        } else {
